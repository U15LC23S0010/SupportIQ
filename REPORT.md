# SupportIQ â€” Evidence-Grounded AI Customer Support Agent
...
2. retrieve historically similar customerâ†’Tesco response examples;
...
The selected Tesco corpus contains 16,722 reconstructed customer-support conversations and 38,468 valid customerâ†’Tesco response relationships before golden-set exclusion.

## 2. Data and evaluation design

The full TWCS dataset contains 2,811,774 tweets.

To avoid evaluation leakage, the historical response corpus was constructed from customerâ†’Tesco response pairs and all conversations belonging to the frozen 172-case golden evaluation set were excluded from retrieval.

The resulting historical response corpus contains:

- 37,855 usable historical customerâ†’Tesco pairs
...
The golden set was intentionally frozen at 172 because the assignment requires 150â€“250 hand-labelled examples.
...
The majority-class baseline provides a trivial lower bound. LinearSVC is the simple learned baseline. Logistic Regression was selected because it performed best on the frozen golden set.
...
The human review exposes an important separation between retrieval and generation.

Historical evidence was often judged useful even when the classifier intent was wrong, but the deterministic reply generator frequently failed to turn that evidence into a specific customer-facing answer.
...
## 6. Top five failure examples

### Failure 1 â€” High-confidence wrong intent with excellent evidence
...
### Failure 2 â€” Product quality misclassification
...
### Failure 3 â€” Availability misclassified as safety
...
### Failure 4 â€” Positive feedback misclassified as safety
...
### Failure 5 â€” Pricing misclassified as availability
...
Only three human-reviewed cases had confidence â‰¥0.70, and two of those three were wrong. That corresponds to a **66.67% error rate among high-confidence cases in this small sample**.
...
### Days 1â€“2: improve intent classification
...
### Days 3â€“4: improve evidence-aware response generation
...
### Day 5: improve confidence and handling calibration
...
### Days 6â€“7: stronger evaluation

Complete the LLM-judge comparison when API access is available.

Run the same 32 cases through the independent judge, calculate human-vs-LLM agreement, and expand the review set if disagreements reveal systematic ambiguity.
...
The LLM judge could not be executed because API credits were unavailable, so human-vs-LLM agreement remains an unexecuted evaluation component rather than a reported result.
